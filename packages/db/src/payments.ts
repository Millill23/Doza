...
export async function settlePaidOrder(
  ...
  if (order.customerId) {
    ...
    //
    // Сертификаты сюда не попадают: считаем по `order.items`, а они лежат
    // отдельной таблицей. Так и надо — за покупку сертификата кешбек не
    // положен: его номинал и так вернётся баллами при активации, и начислять
    // сверху значило бы отдать одну сумму дважды.
    const cashback =
      Math.round(
